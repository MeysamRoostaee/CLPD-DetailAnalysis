...
def longest_common_substring_all(s1, s1_off, s2, s2_off,
                                 th):  # Using Dynamic programming #Necesito encontrar todos los elementos mayor a un umbral en lugar de solo el mayor
    """
...
def tokenize(text, voc={}, offsets=[], sents=[], rem_sw=0):
    ...
    if rem_sw == 0:
        ...
        stopwords = ['the', 'of', 'and', 'a', 'in', 'to', 'is', 'was', 'it', 'for', 'with', 'he', 'be', 'on', 'i',
                     ...
                     'she', 'they', 'or', 'an', 'were', 'we', 'their', 'been', 'has', 'have', 'will', 'would', 'her',
...
def modify_parameters(p, parameters, addr):
    ...
    for i in range(0, len(p), 2):
        if p_list.has_key(p[i]):
            ...
            print
            'Parameter ***', p[i], '*** not recognized!'
            print
            'Use any of this parameters: ', p_list
            exit()
    return 0
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 5:
        ...
        for line in lines:
            ...
            line
            susp, src = line.split()
            sgsplag_obj = SGSPLAG(read_document(os.path.join(suspdir, susp)), read_document(os.path.join(srcdir, src)),
                                  parameters)
