...
class CandidateFragmentIdentification:
    ...
    def get_candidate_fragment(self, RatK, susp_vect, susp_voc, src_vect, src_voc):
        ...
        if RatK>len(src_vect):
            RatK=len(src_vect)

        for c in range(len(susp_vect)):
            arr=sc[c]
            ind = np.argpartition(arr, -RatK)[-RatK:]
            for r in ind:
                if sc[c][r]>0:
                    cf[(c, r)] = sc[c][r]
        ...
        res_bound=self.get_ps_bound(ps)
        return res_bound
