...
class PerformanceDetermination:
    ...
    def extract_annotations_from_files(self, path, tagname,srclang_list, case_length):
        ...
        for xmlfile in xmlfiles:
            annotations.update(self.extract_annotations_from_file(xmlfile, tagname,srclang_list, case_length))
        return annotations


    def extract_annotations_from_file(self, xmlfile, tagname, srclang_list, case_length):
        ...
        for node in doc.documentElement.childNodes:
            if node.nodeType == xml.dom.Node.ELEMENT_NODE and \
               ...
               node.getAttribute('name').endswith(tagname):
                ann = self.extract_annotation_from_node(node, t_ref,srclang_list, case_length)
                if ann:
                    annotations.add(ann)
        return annotations


    def extract_annotation_from_node(self, xmlnode, t_ref,srclang_list, case_length):
        ...
        if xmlnode.hasAttribute('source_reference') and \
           ...
           xmlnode.hasAttribute('source_length'):
            ...
            if xmlnode.hasAttribute('manual_obfuscation'):
                if case_length=="short":
                    if t_len>700:
                        return False
                elif case_length=="medium":
                    if t_len<700 or t_len>5000:
                        return False
                elif case_length=="larg":
                    if t_len<5000:
                        return False
    ...
    def parse_options(self):
        ...
        # det_path = "C:\\Users\\Sahelsoft\\Desktop\\Text Alignment Task\\Dataset\\Det\\outdir-cf\\"+benchmark_name+"\\"+str(k)+"\\"
        ...
        if det_path == "undefined":
            ...
            sys.exit()
        return (micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name, srclang_path,lang,length)


    def main(self, micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name,srclang_path,lang, case_length):
        """Main method of this module."""
        srclang_list=self.read_srclang(srclang_path,lang)
        print('Reading ' + plag_path)

        cases = self.extract_annotations_from_files(plag_path, plag_tag_name,srclang_list,case_length)
        print('Reading ' + det_path)
        detections = self.extract_annotations_from_files(det_path, det_tag_name,srclang_list,case_length)
