...
class PerformanceDetermination:
        ...
        if xmlnode.hasAttribute('source_reference') and \
           ...
           xmlnode.hasAttribute('source_length'):
            ...
            if s_ref not in srclang_list:
                return False
            cur_case=t_ref+'--'+s_ref
            if cur_case not in self.det_src_case:
                self.det_src_case[cur_case]=1
            else:
                self.det_src_case[cur_case]+=1
    ...
    def parse_options(self, benchmark_name, directory, lang):
        ...
        det_path = "C:\\Users\\Sahelsoft\\Desktop\\Text Alignment Task\\Dataset\\Det\\outdir-cf\\"+benchmark_name+"\\"+directory+"\\"
    ...
    def main(self, micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name,srclang_path,lang):
        ...
        print("lang="+lang+"\t\t"+"det_src_case="+str(len(self.det_src_case))+"\t\t"+"total_det="+str(len(detections))+"\t\t"+"avg="+str(len(detections)/len(self.det_src_case)))
...
for subdir,dirs,files in os.walk(det_path):
    for directory in dirs:
        th1=str(format(float(directory.split("--")[0]), '.2f'))
        th2 = str(format(float(directory.split("--")[1]), '.2f'))
        ...
        for lang in lang_list:
            ...
            s=PD.main(*PD.parse_options(benchmark_name, directory, lang))
            score[th1+"--"+th2+"--"+lang]=s

new_score=dict(sorted(score.items()))
print("lang"+"\t\t"+"th1"+"\t\t\t"+"th2"+"\t\t\t"+"rec"+"\t\t\t"+"prec"+"\t\t"+"gran"+"\t\t"+"plag")
print("======================================================================================================")
for key,value in new_score.items():
    keys = key.split("--")
    if keys[2]!=test_lang:
        continue
    print(keys[2]+"\t\t"+keys[0]+"\t\t"+keys[1]+"\t\t"+format(value[0],'.4f')+"\t\t"+format(value[1],'.4f')+"\t\t"+format(value[2],'.4f')+"\t\t"+format(value[3],'.4f'))
    print("======================================================================================================")
