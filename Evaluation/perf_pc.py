...
class PerformanceDetermination:
    ...
    def extract_annotations_from_files(self, path, tagname,srclang_list):
        ...
        for xmlfile in xmlfiles:
            annotations.update(self.extract_annotations_from_file(xmlfile, tagname,srclang_list))
        return annotations


    def extract_annotations_from_file(self, xmlfile, tagname, srclang_list):
        ...
        for node in doc.documentElement.childNodes:
            if node.nodeType == xml.dom.Node.ELEMENT_NODE and \
               ...
               node.getAttribute('name').endswith(tagname):
                ann = self.extract_annotation_from_node(node, t_ref,srclang_list)
                if ann:
                    annotations.add(ann)
        return annotations


    def extract_annotation_from_node(self, xmlnode, t_ref,srclang_list):
    ...
    def parse_options(self, directory, benchmark_name, lang):
        ...
        det_path = "C:\\Users\\Sahelsoft\\Desktop\\Text Alignment Task\\Dataset\\Det\\outdir-ps\\"+benchmark_name+"\\"+directory+"\\"
        ...
        if det_path == "undefined":
            ...
            sys.exit()
        return (micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name, srclang_path,lang)


    def main(self, micro_averaged, plag_path, plag_tag_name, det_path, det_tag_name,srclang_path,lang):
        """Main method of this module."""
        srclang_list=self.read_srclang(srclang_path,lang)
        # print('Reading ' + plag_path)
        cases = self.extract_annotations_from_files(plag_path, plag_tag_name,srclang_list)
        # print('Reading ' + det_path)
        detections = self.extract_annotations_from_files(det_path, det_tag_name,srclang_list)
        ...
        plagd=self.plagdet_score(rec, prec, gran)
        return [rec, prec, gran, plagd]
...
# benchmark="PAN11"
benchmark="PAN12-Test"
#
# test_lang="german"
# test_lang="spanish"
test_lang="both"

det_path = "C:\\Users\\Sahelsoft\\Desktop\\Text Alignment Task\\Dataset\\Det\\outdir-ps\\"+benchmark+"\\"
score={}
for subdir,dirs,files in os.walk(det_path):
    for directory in dirs:
        syn_minscore=str(format(float(directory.split("--")[0]), '.1f'))
        near_minscore = str(format(float(directory.split("--")[1]), '.1f'))
        n_gram = str(directory.split("--")[2])
        min_cliquesize = str(directory.split("--")[3])
        min_matchsize=str(directory.split("--")[4])
        print("syn_minscore= "+syn_minscore+ "\t\t"+"near_minscore = "+near_minscore + "\t\t"+"n_gram = "+n_gram+"\t\t"+"min_cliquesize = "+min_cliquesize+"\t\t"+"min_matchsize= "+min_matchsize)
        lang_list=["german","spanish","both"]
        for lang in lang_list:
            if lang!=test_lang:
                continue
            PD=PerformanceDetermination()
            s=PD.main(*PD.parse_options(directory, benchmark, lang))
            score[syn_minscore+"--"+near_minscore+"--"+n_gram+"--"+min_cliquesize+"--"+min_matchsize+"--"+lang]=s

new_score=dict(sorted(score.items()))
print("lang"+"\t\t"+"syn_minscore"+"\t\t"+"near_minscore"+"\t\t"+"n_gram"+"\t\t"+"min_cliquesize"+"\t\t"+"min_matchsize"+"\t\t"+"rec"+"\t\t"+"prec"+"\t\t"+"gran"+"\t\t"+"plag")
print("======================================================================================================")
for key,value in new_score.items():
    keys = key.split("--")
    if keys[5]!=test_lang:
        continue
    print(keys[5]+"\t\t"+keys[0]+"\t\t\t\t"+keys[1]+"\t\t\t\t\t\t"+keys[2]+"\t\t\t\t\t\t"+keys[3]+"\t\t\t\t\t\t"+keys[4]+"\t\t\t"+format(value[0],'.4f')+"\t\t"+format(value[1],'.4f')+"\t\t"+format(value[2],'.4f')+"\t\t"+format(value[3],'.4f'))
    print("======================================================================================================")
